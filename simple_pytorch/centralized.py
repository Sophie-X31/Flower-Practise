...
def train(net, trainloader, epochs):
    ...
    for i in range(epochs):
        for batch in tqdm(trainloader, "Training"):
            ...
            optimizer.step()
            print(f'epoch {i+1}/{epochs}, loss: {criterion(net(images.to(DEVICE)), labels.to(DEVICE)).item()}')
...
def load_data():
    ...
    partition_train_test = partition.train_test_split(test_size=0.2, seed=42) # Divide data on each node: 80% train, 20% test
